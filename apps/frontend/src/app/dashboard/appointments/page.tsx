...
const UserAppointments: React.FC = () => {
  ...
  const handleAppointmentSubmit = (appointmentData: { realtorName: string; propertyAddress: string; date: string; time: string; message?: string }) => {
    const newAppointment = {
      ...
      date: new Date(appointmentData.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }),
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <div className="space-y-4 max-h-96 overflow-y-auto">
                {myAppointments.length === 0 ? (
                  <p className="text-sm text-gray-500 text-center py-8">No appointments scheduled yet.</p>
                ) : (
